...
Wallet* create_acc(Blockchain &bc)
{
    ...
    cout << "Step 1: Set a Transaction Encryption Key: ";
    cin >> enc_key;
...
int get_balance(Blockchain &bc, string pub)
{
    ...
    while(temp != NULL)
    {
        ...
        if(p1 != string::npos && p2 != string::npos)
        {
            ...
            // Handle if there's a 3rd pipe (transfer format: sender|receiver|amount|ts)
            ...
            // ensure valid number
            if(!amountStr.empty() && amountStr.find_first_not_of("0123456789") == string::npos)
            {
