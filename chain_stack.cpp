  ...
  void insert_tail(vector<unsigned char> encryptedData) {
    ...
    if (LOADING_MODE) {
      ...
      while (true) {
        ...
        bool ok = true;
        for (int i = 0; i < DIFFICULTY && i < (int)newnode->hash.length();
             i++) {
          if (newnode->hash[i] != '0') {
            ok = false;
            break;
...
// Compile:
// cd Blockchain
// g++ chain_stack.cpp -o chain_stack.exe -IC:\Users\User\anaconda3\Library\include -LC:\Users\User\anaconda3\Library\lib -lssl -lcrypto -lws2_32 -lgdi32 -lcrypt32
// .\chain_stack.exe